...
bool NodeConnectionInteraction::_chkLoop(std::set<PortIndex> *set, PortIndex index) {}
...
bool NodeConnectionInteraction::canConnect(PortIndex *portIndex) const
{
    ...
    std::set<ConnectionId> marked_edge;
    std::set<PortIndex> marked_port;
    marked_port.insert(connectedNodeId);
    marked_port.insert(_ngo.nodeId());
    auto& all_edge = this->_scene._connectionGraphicsObjects;

    int turns = 1;
    printf("Total=%d\n", all_edge.size());
    while(turns) {
        turns = 0;
        for (auto& it : all_edge) {
            // 跳过遍历过的边
            bool flag = false;
            for(auto& k : marked_edge) {
                // first == ConnectionId
                if (it.first == k) {
                    flag = true;
                    break;
            ...
            // 检查端点
            int is_from = marked_port.count(it.first.inNodeId);
            int is_to   = marked_port.count(it.first.outNodeId);
            if (is_from && is_to) // 检查到环
                return false;
            if (is_from || is_to) { // 有关边
                marked_port.insert(it.first.inNodeId);
                marked_port.insert(it.first.outNodeId);
                marked_edge.insert(it.first);
                turns ++;
            }
        }
        printf("edge = %d\n", turns);
